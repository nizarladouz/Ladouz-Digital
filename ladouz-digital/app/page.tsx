...
/* Bildflächen: `bild` leer lassen → CI-Grafik wird gerendert.
   Sobald ein Motiv vorliegt, Pfad eintragen (z. B. "/hero-ki.jpg").
   Empfehlung: 2400 × 1600 px, ruhige Bildmitte, Motiv leicht rechts. */
const heroSlides = [
  {
    kicker: "Analyse",
...
function SectionTitle({ children, tone = "dark", className = "" }: { children: ReactNode; tone?: "dark" | "light"; className?: string }) {
  return (
    <h2 className={`font-head mt-5 text-[clamp(2.05rem,3.9vw,3.15rem)] font-bold leading-[1.08] tracking-[-0.03em] ${tone === "light" ? "text-white" : "text-[#0b1233]"} ${className}`}>
      {children}
    </h2>
  );
}
...
function Lead({ children, tone = "dark", className = "" }: { children: ReactNode; tone?: "dark" | "light"; className?: string }) {
  return (
    <p className={`mt-6 max-w-[60ch] text-[1.08rem] font-light leading-[1.78] ${tone === "light" ? "text-[#a7b4d6]" : "text-[#43507a]"} ${className}`}>
      {children}
    </p>
  );
}
...
/* ══════════════════════════ Visual ══════════════════════════
   Bildfläche mit Ken-Burns-Zoom – die Bewegung, die grosse
   Beratungshäuser hinter ihren Headlines einsetzen.

   Ohne `src` wird eine CI-Grafik als SVG gerendert (aktueller Stand).
   Sobald ein Foto vorliegt, genügt:

       <Visual variant="wave" src="/hero-1.jpg" alt="…" />

   Das Bild wird dann per next/image ausgeliefert und langsam
   herangezoomt. Empfehlung fuer Motive: 2400 × 1600 px, JPEG oder
   WebP, ruhige Bildmitte, da der Text darueber liegt.
   ═════════════════════════════════════════════════════════════ */
...
function Visual({
  ...
  if (src) {
    return (
      <div className={`relative overflow-hidden bg-[#0b1233] ${className}`}>
        ...
        />
        <span aria-hidden className="pointer-events-none absolute inset-0 bg-[linear-gradient(0deg,rgba(11,18,51,.55)_0%,rgba(11,18,51,.12)_60%)]" />
      </div>
    );
  }
  ...
  return (
    <div className={`relative overflow-hidden bg-[linear-gradient(126deg,#0b1233_0%,#131f5c_45%,#2f5bd7_100%)] ${className}`} role="img" aria-label={alt || "Grafik im Ladouz-Design"}>
      <svg viewBox="0 0 800 600" preserveAspectRatio="xMidYMid slice" className="ld-kenburns absolute inset-0 h-full w-full">
        <defs>
        ...
        <div className="relative z-10 px-6 py-[86px] lg:py-[124px]">
          ...
          <div className="relative">
            ...
            <Reveal key={`t-${i}`} delay={80}>
              <h1 className="font-head mt-6 max-w-[18ch] text-[clamp(2.3rem,4.7vw,3.8rem)] font-bold leading-[1.04] tracking-[-0.035em]">
                {slide.titel}
              </h1>
            </Reveal>

            <Reveal key={`x-${i}`} delay={160}>
              <p className="mt-6 max-w-[50ch] text-[1.1rem] font-light leading-[1.78] text-[#a7b4d6]">{slide.text}</p>
            </Reveal>
...
function GlobalStyles() {
  const css = `
    ...
    /* ── Typografie ────────────────────────────────────────────
       Jost traegt die gesamte Seite. Die geometrische Anatomie
       entspricht der Logo-Konstruktion; Garet waere der direkte
       Verwandte, ist aber kein freier Webfont (siehe layout.tsx).

       Jost hat eine kleinere x-Hoehe als Grotesk-Schriften.
       Deshalb: Grundgroesse leicht angehoben, Zeilenhoehe grosszuegig,
       Headlines enger getrackt, Versal-Labels weiter geoeffnet.
       ────────────────────────────────────────────────────────── */
    .font-head, .font-body, .font-display,
    ...
    .font-head { letter-spacing: -.022em; }
    .font-display { font-variant-numeric: tabular-nums; }
    ...
    /* Ken-Burns: langsamer Zoom hinter Headlines */
    .ld-kenburns {
